...
#include "HAL/pins/am3358zcz_pins.hpp"
...
namespace HAL::SERIAL
{
    ...
    void  serial::module_state_t::update()
    {
        ...
        module_function = static_cast<e_MODESELECT>(m_instance.MDR1.b.MODESELECT);
        ...
        enhanced_sts = static_cast<e_ENH>(m_instance.EFR.b.ENHANCEDEN);
        ...
        tLCR.reg = m_instance.LCR.reg;
        m_instance.LCR.reg = (static_cast<uint32_t>(CONFIG_MODE_A) & 0xFF);
     ...
     */
    void  serial::resume_operation() const
    {
    ...
    void serial::init(serial_user_callback usr_clb)
    {
        ...
        while(wkup.UART0_CLKCTRL.b.IDLEST != IDLEST_FUNC){}   // waiting for fully enabled
        ...
        while(wkup.L4WKUP_CLKCTRL.b.IDLEST != IDLEST_FUNC){}  // waiting for fully enabled
...
#define SWITCH_OPERMODE(LCR) (LCR &= 0x7F)
    ...
    * 'mode' can take one of the following values:
    ...
    void serial::hexdump(uint32_t val)
    {
        ...
        for (i = 7; i >= 0; i--)
        {
            put_char(m_hexchars[(val >> (i * 4)) & 0xF]);
        }
    }
    ...
    static void uart_isr(void *p_obj)
    {
        ...
        char received = REGS::UART::AM335X_UART_0->RHR.reg;
