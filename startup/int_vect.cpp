...
namespace
{
  ...
  constexpr std::size_t number_of_interrupts     = std::size_t(128U);
}
...
void __undef_instr_handler() { for(;;) { nop(); } }
void __pend_sv_handler    () { for(;;) { nop(); } }
void __abort_handler      () { for(;;) { nop(); } }
void __vector_unused_irq  () { for(;;) { nop(); } }
